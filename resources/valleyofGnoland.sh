...
GNOLAND_ACTIVE_REALM="r/sys/validators/v3"
GNOLAND_VALOPER_REALM="r/gnops/valopers"
VALOPER_GAS_FEE="1000000ugnot"
...
function operator_key_address() {
    gnokey -home "$GNOKEY_HOME" list 2>/dev/null |
        awk -v key="$1" '$2 == key && $5 == "addr:" { print $6; exit }'
}

function get_valoper_register_fee() {
    local fee_output fee
    fee_output=$(gnokey_cmd query vm/qeval -data 'gno.land/r/sys/params.GetValoperRegisterFee()' 2>/dev/null) || return 1
...
function local_consensus_pubkey() {
    "$GNOLAND_BIN" secrets get --data-dir "$GNOLAND_TESTNET_HOME/secrets" validator_key 2>/dev/null |
        grep -Eo 'gpub1[0-9a-z]+' | head -n 1
}

function show_validator_pubkey() {
    local key_name operator_addr local_pubkey profile registered_pubkey signing_addr active_output candidate_status active_status identity_status
    ...
    if [ -n "$registered_pubkey" ]; then
        ...
        if [ -n "$signing_addr" ] && printf '%s\n' "$active_output" | grep -Fq "$signing_addr"; then
            active_status="YES"
        fi
...
function show_account_dashboard() {
    ...
    local account_number sequence profile registered_pubkey signing_addr active_output candidate_status active_status
...
function register_valoper_candidate() {
    ...
    local tx_output suggested_gas retry_confirm
    ...
    case "$INFRA_TYPE" in
        ...
        *)
            echo -e "${RED}Infrastructure type must be exactly: cloud, on-prem, or data-center.${RESET}"
            echo -e "${YELLOW}Press Enter to go back to main menu${RESET}"
            read -r
            menu
            return
    ...
    profile=$(valoper_profile_output "$OPERATOR_ADDR" || true)
    if [ -n "$(profile_signing_pubkey "$profile")" ]; then
        echo -e "${RED}Registration blocked: this operator address already has a valoper profile.${RESET}"
        printf '%s\n' "$profile"
        echo -e "${YELLOW}Use option 2e to manage the existing profile.${RESET}"
        echo -e "${YELLOW}Press Enter to go back to main menu${RESET}"
        read -r
        menu
        return
...
function manage_valoper_profile() {
    ...
    operator_addr=$(operator_key_address "$key_name")
    profile=$(valoper_profile_output "$operator_addr" || true)
    if [ -z "$(profile_signing_pubkey "$profile")" ]; then
            ...
            if ! operator_key_exists "$key_name"; then
                ...
                if [ -z "$(profile_signing_pubkey "$profile")" ]; then
                    ...
                    if ! [[ "$rotation_fee" =~ ^[0-9]+$ ]]; then
                        ...
                        echo -e "${RED}Rotation blocked: current on-chain rotation fee is ${rotation_fee} ugnot.${RESET}"
