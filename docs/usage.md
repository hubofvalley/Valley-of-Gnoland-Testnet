...
| **1e. Show Node Status** | Prints an operational health summary: systemd state, local RPC endpoint, node directory disk usage, local height, public network height, block difference, catching-up flag, connected peer count, latest block time, validator address, and copy-ready debug commands. | Check sync progress before candidate registration or collect fast triage output. |
