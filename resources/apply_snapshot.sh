...
check_dependencies() {
    ...
    if [ "${#missing_packages[@]}" -gt 0 ]; then
        echo -e "${YELLOW}Installing required dependencies: ${missing_packages[*]}${RESET}"
        sudo apt-get update
...
validate_runtime_targets() {
    local resolved_home resolved_source resolved_user_home
    resolved_home=$(readlink -m -- "$GNOLAND_TESTNET_HOME")
    resolved_source=$(readlink -m -- "$GNO_SOURCE_DIR")
    resolved_user_home=$(readlink -m -- "$HOME")

    case "$resolved_home" in
        /|"$resolved_user_home"|"$resolved_source")
            echo -e "${RED}Unsafe GNOLAND_TESTNET_HOME rejected: $resolved_home${RESET}" >&2
            return 1
            ;;
        "$resolved_user_home"/*) ;;
        *)
    ...
    if [[ ! "$GNOLAND_TESTNET_SERVICE_NAME" =~ ^[A-Za-z0-9][A-Za-z0-9_.@-]*$ ]]; then
        echo -e "${RED}Invalid Gnoland testnet service name: $GNOLAND_TESTNET_SERVICE_NAME${RESET}" >&2
        return 1
    fi
...
' <<<"$index_json" 2>/dev/null); then
    ...
    if [ "$SNAPSHOT_URL" != "$HAZEN_STABLE_URL" ]; then
        SNAPSHOT_URL="$HAZEN_STABLE_URL"
        if fetch_http_headers "$SNAPSHOT_URL" >/dev/null; then
            SNAPSHOT_AVAILABLE=1
            return 0
...
show_snapshot_stats() {
    ...
    if [ "$SNAPSHOT_AVAILABLE" -eq 1 ]; then
        ...
        status="${RED}Not available at the moment${RESET}"
    fi

    echo -e "${GREEN}${SNAPSHOT_PROVIDER}${RESET}"
...
ask_database_backup() {
    echo -e "${YELLOW}Applying a snapshot will replace the current db and wal folders.${RESET}"
    read -r -p "Backup current db and wal first? (y/n): " backup_choice
    case "${backup_choice,,}" in
        y|yes) return 0 ;;
        n|no) return 1 ;;
        *)
            echo -e "${RED}Invalid choice. Snapshot application cancelled.${RESET}"
            exit 1
            ;;
    esac
}
...
backup_current_database() {
    local backup_items=() backup_file
    ...
    if [ "${#backup_items[@]}" -eq 0 ]; then
        echo -e "${YELLOW}No db or wal folders found to backup.${RESET}"
        return 0
    fi

    echo -e "${GREEN}Backing up current database...${RESET}"
    tar -czf "$backup_file" -C "$GNOLAND_TESTNET_HOME" "${backup_items[@]}" || return 1
    chmod 600 "$backup_file" || return 1
    echo -e "${GREEN}Backup saved to:${RESET} $backup_file"
}

verify_snapshot_archive() {
    local archive=$1 archive_listing

    if [ -n "$SNAPSHOT_SHA256" ]; then
        echo -e "${GREEN}Verifying SHA-256 checksum...${RESET}"
        printf '%s  %s\n' "$SNAPSHOT_SHA256" "$archive" | sha256sum --check --status
    else
        echo -e "${YELLOW}Provider does not publish a checksum; checksum verification skipped.${RESET}"
    fi

    echo -e "${GREEN}Validating snapshot archive paths...${RESET}"
...
stop_gnoland() {
    ...
    if systemctl is-active --quiet "$GNOLAND_TESTNET_SERVICE_NAME"; then
        echo -e "${GREEN}Stopping Gnoland service...${RESET}"
        sudo systemctl stop "$GNOLAND_TESTNET_SERVICE_NAME"
    fi
}

start_gnoland() {
    echo -e "${GREEN}Restarting Gnoland service...${RESET}"
...
rollback_database() {
    echo -e "${RED}Snapshot activation failed; restoring the previous db and wal.${RESET}"
...
activate_snapshot() {
    local archive=$1 should_backup=$2
    ...
    if [ "$should_backup" -eq 1 ]; then
        if ! backup_current_database; then
            echo -e "${RED}Database backup failed; restarting Gnoland without applying the snapshot.${RESET}"
            sudo systemctl restart "$GNOLAND_TESTNET_SERVICE_NAME" || true
            return 1
        fi
    fi
    ...
    echo -e "${GREEN}Extracting the verified snapshot...${RESET}"
...
apply_snapshot() {
    ...
    if ! "$loader"; then
        show_snapshot_stats
        echo -e "${RED}Snapshot application cancelled because the provider is unavailable.${RESET}"
        exit 1
    fi

    show_snapshot_stats
    echo -e "Grand Valley extends its gratitude to ${YELLOW}$SNAPSHOT_PROVIDER${RESET} for providing snapshot support."
    echo -e "${YELLOW}Config and node secrets are kept in place.${RESET}"
    read -r -p "Apply this snapshot now? Type yes to continue: " confirm
    if [[ "${confirm,,}" != "yes" ]]; then
        echo -e "${RED}Snapshot application cancelled.${RESET}"
        exit 0
    fi
    ...
    echo -e "${GREEN}Downloading snapshot before stopping Gnoland...${RESET}"
    ...
    echo -e "${GREEN}Snapshot setup completed successfully.${RESET}"
    echo -e "${YELLOW}Showing live logs. Press Ctrl+C to stop following logs.${RESET}"
